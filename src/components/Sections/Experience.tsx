            ...
            >
              ...
              <div className={`flex flex-col md:flex-row justify-between items-start md:gap-12 ${index % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
                ...
                <div className="md:w-[45%] mb-4 md:mb-0 relative z-10 order-2 md:order-none w-full">
                  <div className="p-6 bg-surface border border-border rounded-xl hover:border-secondary transition-all duration-300 group-hover:-translate-y-1 shadow-sm hover:shadow-primary/5 relative overflow-hidden">
                    {/* Subtle hover gradient background */}
                    <div className="absolute inset-0 bg-gradient-to-br from-primary/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"></div>
                    
                    <div className="flex flex-col mb-4 relative z-10">
                      <h3 className="text-xl font-bold text-primary">{exp.role}</h3>
                      <h4 className="text-secondary font-medium">{exp.company}</h4>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 relative z-10">
                      {exp.technologies.map(tech => (
                        <span key={tech} className="text-xs font-mono text-secondary bg-surfaceHighlight border border-border px-2 py-1 rounded">
                          {tech}
                        </span>
                      ))}
                    </div>
                ...
                <div className="absolute left-0 md:static md:w-[10%] flex md:justify-center z-20 mt-1 md:mt-0">
                   ...
                   <div className="absolute top-6 left-[3px] md:left-1/2 md:-translate-x-1/2 w-4 h-4 bg-primary/20 rounded-full blur-md animate-pulse"></div>
                   
                   {/* Hover Ripple - Expands on hover */}
                   <div className="absolute top-6 left-[3px] md:left-1/2 md:-translate-x-1/2 w-4 h-4 rounded-full border border-primary opacity-0 transition-all duration-700 group-hover:opacity-100 group-hover:scale-[2.5] group-hover:border-opacity-0 ease-out"></div>

                   {/* Actual Dot */}
                   <div className="absolute top-6 left-[3px] md:left-1/2 md:-translate-x-1/2 w-4 h-4 bg-background rounded-full border-[3px] border-primary shadow-[0_0_10px_rgba(var(--color-primary),0.2)] transition-all duration-300 group-hover:scale-125 group-hover:bg-primary group-hover:border-background group-hover:shadow-[0_0_20px_rgba(var(--color-primary),0.4)] z-10"></div>
                </div>

                {/* Date Side */}
                <div className={`md:w-[45%] flex items-center md:py-0 order-1 md:order-none mb-2 md:mb-0 ${index % 2 === 0 ? 'justify-end' : 'justify-start'}`}>
                  <span className="font-mono text-sm text-secondary block group-hover:text-primary transition-colors duration-300">
                    {exp.period}
                  </span>
                </div>
